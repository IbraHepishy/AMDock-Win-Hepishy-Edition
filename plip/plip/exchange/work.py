...
def xml_to_xlsx(xml_file_path, output_path=None):
    ...
    for bsite in plip_data.bsites.values():
        ...
        properties = [
            ...
            ('LogP', bsite.logp),
            ('Rotatable Bonds', bsite.rotatable_bonds),
            ('Aromatic Rings', bsite.rings)
        ]
        ...
        for i, header in enumerate(headers):
            worksheet.write(current_row, i, header, formats['section_header'])
        current_row += 1
        ...
        # Add a summary of binding site residues
        if bsite.bs_res:
            worksheet.merge_range(current_row, 0, current_row, 4, 
                                'Binding Site Residues', formats['section_header'])
            current_row += 1
            
            bs_res_headers = ['Residue', 'Chain', 'Amino Acid', 'Contact', 'Min Distance (Å)']
            for i, header in enumerate(bs_res_headers):
                worksheet.write(current_row, i, header, formats['section_header'])
            current_row += 1
            
            for res in bsite.bs_res:
                worksheet.write(current_row, 0, str(res['resnr']), formats['normal'])
                worksheet.write(current_row, 1, res['reschain'], formats['normal'])
                worksheet.write(current_row, 2, res['aa'], formats['normal'])
                worksheet.write(current_row, 3, "Yes" if res['contact'] else "No", formats['normal'])
                worksheet.write(current_row, 4, res['min_dist'], formats['number'])
                current_row += 1
