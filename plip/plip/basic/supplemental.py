...
def int32_to_negative(int32):
    ...
    """
    if int32 == 4294967295:  # Special case in some structures
        return -1
    
    # Convert the input number to a signed int32 if it represents a negative number
    if int32 > 0x7FFFFFFF:  # If number is larger than max positive 32-bit int
        return np.int32(int32).item()
