...
from plip.exchange.xml_to_xlsx import xml_to_xlsx
...
def process_pdb(pdbfile, outpath, as_string=False, outputprefix='report'):
    ...
    if config.EXCEL:  # Generate report in Excel format
            xml_to_xlsx(xml_file_path=os.path.join(outpath,'report.xml'))
...
def main():
    ...
    parser.add_argument("-t", "--txt", dest="txt", default=False, help="Generate report file in TXT (RST) format",
                        action="store_true")
    parser.add_argument("-P", "--excel", dest="excel", default=False, help="Generate report file in Excel format", action="store_true")
    ...
    parser.add_argument("--name", dest="outputfilename", default="report",
                        help="Set a filename for the report files. Will only work when processing single structures.")
